...
% Nov 4, 2014 - Add the jsd to the anode; Move Li-ion diffusion part to 
%               the end of the calculation to include jsd in feedback.

function [f, g, varargout] = dae_dfn_zhouxinme2(x, z, Cur, p)
...
% Molar ionic flux
jp = z(2*Nnp+Nx+1 : 2*Nnp+Nx+Np);

jn1 = z(2*Nnp+Nx+Np+1 : 2*Nnp+Nx+Nn+Np);
jsd = z(2*Nnp+Nx+Nn+Np+1 : 2*Nnp+Nx+2*Nn+Np);

% Temperature
T = x(end);

%% Li Diffusion in Solid Phase I: c_s(x,r,t) [Current Step]
% Preallocate
c_ss_n = zeros(Nn,1);
c_ss_p = zeros(Np,1);
c_avg_n = zeros(Nn,1);
...
i_en_dot = p.F1_ien*i_en + p.F2_ien*(jn1+jsd) + p.F3_ien*Cur;
...
Unref = refPotentialAnode_LiFePO4_zhouxinme(p, c_ss_n / p.c_s_n_max);
Upref = refPotentialCathode_LiFePO4_zhouxinme(p, c_ss_p / p.c_s_p_max);

% Overpotential, \eta
eta_n = phi_s_n - phi_e(1:Nn) - Unref - p.Faraday*p.R_f_n*(jn1+jsd);
eta_p = phi_s_p - phi_e(end-Np+1:end) - Upref - p.Faraday*p.R_f_p*jp;

% Algebraic eqns for cathode (semi-explicit form)
jp_dot = 2/p.Faraday * i_0p .* sinh(aFRT * eta_p) - jp;

% Algebraic eqns for anode (semi-explicit form)
jn1_dot = 2/p.Faraday * i_0n .* sinh(aFRT * eta_n) - jn1;

% Health: side reaction Butler-Volmer equation
eta_sd = phi_s_n - phi_e(1:Nn) - p.Urefsd - p.Faraday*p.R_f_n*(jn1+jsd);
% jsd_dot = -p.i_0sd*p.a_s_n*exp(-aFRT * eta_sd) - jsd;
jsd_dot = - p.i_0sd /p.Faraday *exp(-aFRT * eta_sd) - jsd;

%% Temperature
% Equilibrium Potential and Gradient wrt bulk concentration
[Unb,trash_var,dUnbdT] = refPotentialAnode_LiFePO4_zhouxinme(p, c_avg_n / p.c_s_n_max);
[Upb,trash_var,dUpbdT] = refPotentialCathode_LiFePO4_zhouxinme(p, c_avg_p / p.c_s_p_max);

% Heat generated from intercalation (w/o boundaries for NOW)
Q_nx = p.a_s_n*p.Faraday * (jn1+jsd) .* (Unb - T*dUnbdT);
...
%% Li Diffusion in Solid Phase II: c_s_dot_mat(x,r,t) [Next Step]
...
% Loop through each "comb tooth" in anode
for idx = 1:Nn
    c_s_n_dot_mat(:,idx) = p.A_csn * c_s_n_mat(:,idx) + p.B_csn * jn1(idx);
end
...
nLidot = sum((jn1+jsd)) * p.delta_x_n * Nn + sum(jp) * p.delta_x_p * Np;
