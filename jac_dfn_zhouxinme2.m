...
function [f_x_full, f_z_full, g_x_full, g_z_full, varargout] = jac_dfn_zhouxinme2(x,z,Cur,f_x,f_z,g_x,g_z,p)
...
[Unb,dUnb] = refPotentialAnode_LiFePO4_zhouxinme(p, c_avg_n / p.c_s_n_max);
[Upb,dUpb] = refPotentialCathode_LiFePO4_zhouxinme(p, c_avg_p / p.c_s_p_max);

% Derivatives wrt c_s
dfTdcsn_int = -(p.a_s_n*p.Faraday * (jn1+jsd) .* dUnb) / (p.rho_avg*p.C_p);
...
% Derivatives wrt jn, jp
dfTdjn_int = -(p.a_s_n*p.Faraday * Unb)/(p.rho_avg*p.C_p);
dfTdjn = dfTdjn_int * p.delta_x_n * p.L_n;   % jn affects the temperature; jn = jn1 + jsd
...
f_z(end,ind_jn1) = dfTdjn';
f_z(end,ind_jsd) = dfTdjn';
...
[Unref,dUnref] = refPotentialAnode_LiFePO4_zhouxinme(p, c_ss_n / p.c_s_n_max);
[Upref,dUpref] = refPotentialCathode_LiFePO4_zhouxinme(p, c_ss_p / p.c_s_p_max);

% Overpotential
eta_n = phi_s_n - phi_e(1:Nn) - Unref - p.Faraday*p.R_f_n* (jn1+jsd);
eta_p = phi_s_p - phi_e(end-Np+1:end) - Upref - p.Faraday*p.R_f_p* jp;

eta_sd = phi_s_n - phi_e(1:Nn) - p.Urefsd - p.Faraday*p.R_f_n* (jn1+jsd);
...
df6jn1 = -2/p.Faraday*i_0n.*cosh(aFRT*eta_n) * aFRT*p.R_f_n*p.Faraday - 1;   % BV: jn became jn1 when there is jsd
% dfsd6jsd = -p.i_0sd*p.a_s_n*exp(-aFRT * eta_sd)*(aFRT)*p.R_f_n*p.Faraday - 1;
dfsd6jsd = -p.i_0sd/p.Faraday*exp(-aFRT * eta_sd)*(aFRT)*p.R_f_n*p.Faraday - 1;

dfsd6psn = -p.i_0sd*p.a_s_n*exp(-aFRT * eta_sd)*(-aFRT);
dfsd6pen = -p.i_0sd*p.a_s_n*exp(-aFRT * eta_sd)*(aFRT);
...
g_x(ind_jn1,ind_csn) = blkdiagFast(rsn, csn, Cell_df6n{:});   % The effect of z_csn on g_jn1
% The effect of z_csn on g_jsd: No effect, because i0sd is not affected by csn.
g_x(ind_jp,ind_csp) = blkdiagFast(rsp, csp, Cell_df6p{:});

g_x(ind_jn1,ind_ce(1:Nn)) = diag(df6dcen);   % The effect of z_ce on g_jn1
% The effect of z_ce on g_jsd: No effect, because i0sd is not affected by ce.
g_x(ind_jp,ind_ce(end-Np+1:end)) = diag(df6dcep);

g_z(ind_jn1,ind_phi_s_n) = diag(df6psn);   % The effect of z_phisn on g_jn1
g_z(ind_jsd,ind_phi_s_n) = diag(dfsd6psn);   % The effect of z_phisn on g_jsd
g_z(ind_jp,ind_phi_s_p) = diag(df6psp);

g_z(ind_jn1,ind_phi_e(1:Nn)) = diag(df6pen);   % The effect of z_phie on g_jn1
g_z(ind_jsd,ind_phi_e(1:Nn)) = diag(dfsd6pen);   % The effect of z_phie on g_jsd
g_z(ind_jp,ind_phi_e(end-Np+1:end)) = diag(df6pep);

g_z(ind_jn1,ind_jn1) = diag(df6jn1);   % The effect of z_jn1 on g_jn1
g_z(ind_jsd,ind_jsd) = diag(dfsd6jsd);   % The effect of z_jsd on g_jsd
