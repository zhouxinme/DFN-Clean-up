...
% Nov 5, 2014 - The calculation of n_Li_s should contain Area.

function [csn0,csp0] = init_cs_zhouxinme(p,V)
...
for idx = 1:maxiters
    ...
    OCPn = refPotentialAnode_LiFePO4_zhouxinme(p,theta_n);
    OCPp = refPotentialCathode_LiFePO4_zhouxinme(p,theta_p);
...
%% Compute csn0
csn0 = (p.n_Li_s - p.epsilon_s_p * p.L_p * csp0 * p.Area) / (p.epsilon_s_n * p.L_n * p.Area);